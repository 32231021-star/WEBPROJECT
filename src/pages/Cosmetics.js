...
import { CosmeticsList } from "../Data/CosmeticsList";

const generatePrices = (count) => {
  return Array.from({ length: count }, () =>
    (Math.random() * (10 - 3) + 3).toFixed(2)
  );
};

const Cosmetics = ({ addToCart }) => {
  const [prices] = useState(generatePrices(CosmeticsList.length));
  const [quantities, setQuantities] = useState(
    Array(CosmeticsList.length).fill(1)
  );
